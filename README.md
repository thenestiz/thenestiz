
<div align="center">

<img width="1500" height="500" alt="NestIZ" src="https://github.com/user-attachments/assets/9ed6656b-56eb-4a1c-a174-2d6d5096fb53" />


<h1>NestIZ</h1>
<p><em>Design decisions, already made.</em></p>

</div>

<br/>

<div align="center">

<h3>🔗 Connect with us</h3>

<a href="https://nestiz.vercel.app/" target="_blank" rel="noopener noreferrer">
  <img src="https://img.shields.io/badge/-Website-0A84FF?style=for-the-badge&logo=googlechrome&logoColor=white" alt="Website" />
</a>
<a href="https://x.com/thenestiz" target="_blank" rel="noopener noreferrer">
  <img src="https://img.shields.io/badge/-X-000000?style=for-the-badge&logo=x&logoColor=white" alt="X" />
</a>

</div>

<br/>

<h2>About NestIZ</h2>

<p>
NestIZ builds production-ready React website templates for founders and developers
who need to ship fast without starting from a blank file.
</p>

<p>
Every template is built with clean architecture, modern UI standards, and
long-term maintainability in mind — not just something that looks good in a screenshot.
</p>

<br/>


<div align="center">
  
<h2>Technologies</h2>
<br>
<h3>Core Technologies</h3>

<p>
<img src="https://img.shields.io/badge/-HTML5-E34F26?style=flat-square&logo=html5&logoColor=white" alt="HTML5" />
<img src="https://img.shields.io/badge/-CSS3-1572B6?style=flat-square&logo=css3&logoColor=white" alt="CSS3" />
<img src="https://img.shields.io/badge/-JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black" alt="JavaScript" />
<img src="https://img.shields.io/badge/-TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white" alt="TypeScript" />
<img src="https://img.shields.io/badge/-Figma-F24E1E?style=flat-square&logo=figma&logoColor=white" alt="Figma" />
<img src="https://img.shields.io/badge/-GitHub-181717?style=flat-square&logo=github&logoColor=white" alt="GitHub" />
</p>

<br> 

<h3>Frameworks & Libraries</h3>
<p>
<img src="https://img.shields.io/badge/-React-61DAFB?style=flat-square&logo=react&logoColor=black" alt="React" />
<img src="https://img.shields.io/badge/-Next.js-000000?style=flat-square&logo=next.js&logoColor=white" alt="Next.js" />
<img src="https://img.shields.io/badge/-Tailwind_CSS-06B6D4?style=flat-square&logo=tailwindcss&logoColor=white" alt="Tailwind CSS" />
</p>

<br/><br/>
</div>
<h2>How We Build</h2>

<p>
Every template ships with clean component architecture, consistent naming conventions,
and documentation clear enough for you to customize without guesswork.
</p>

<p>
Live demos are linked in each repository below, so you can try before you buy.
</p>

<br/>

<h2>Templates</h2>

<p>
Browse our templates in the pinned repositories below. Each one includes a live demo
link and full feature breakdown. Full source is available on purchase via
<a href="https://thenestiz.gumroad.com/">Gumroad</a>.
</p>

<br/>

<div align="center">
<sub>Website — coming soon</sub>
</div>
